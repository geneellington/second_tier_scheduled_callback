...
// --------------------------------------------------------------------
  ...
  try {
    ...
    // (Keep your existing validations for date/time/required fields)

    const cfg = await loadCapacity();                 // <- read once & cache
    const { queueName, queueArn } = resolveQueue(cfg, groupKey);

    // Build your item with the new attributes
    const item = {
      // your existing attributes...
      id,                       // however you generate it
      date,                     // UTC YYYY-MM-DD
      time,                     // UTC HH:MM
      ...
      notes: body.notes,
      scheduledAt: body.scheduledAt, // if you store it
      groupKey,                 // NEW
      queueName,                // NEW
      queueArn,                 // NEW
    ...
    if (resource === "/entries" && httpMethod === "POST") {
      ...
      // Map groupKey -> queue fields from capacity.json (validate first)
      if (!body.groupKey) {
      ...
      try {
        ...
        // e.g., unknown groupKey in capacity.json
        return res(event, 400, { message: e.message });
      }

      // Map groupKey -> queue fields from capacity.json
      const cfg = await loadCapacity();
      const { queueName, queueArn } = resolveQueue(cfg, body.groupKey);
